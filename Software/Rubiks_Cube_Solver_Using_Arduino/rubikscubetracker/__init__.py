...
import serial 
port = ['/dev/ttyACM0','/dev/ttyUSB1','/dev/ttyUSB0']
baud = 9600
ser = ""
for i in port:
    try:
        ser = serial.Serial(i,baud,timeout=1)
        break
    except:
        continue
if ser.isOpen():
...
def sendSolutionToArduino(str):
    ...
    moves = [{"R": ["z1x"],"R'": ["zi2yiz"],"R2":["z1r1yrz"],
              "L":["x1z"],"L'":["xi2yix"],"L2":["x1r1yrx"],
              "U":["y1y"],"U'":["yi2yi"],"U2":["y1r1yr"],
              "D":["1"],"D'":["i2yiy"],"D2":["1r1yry"],
              "F":["izr1xrz","xizr1xry","yizr1xrx","zizr1xr"],
              "F'":["iz2ziy","xiz2zix","yiz2zi"],
              "F2":["izr1r1xry","xizr1r1xrx","yizr1r1xr","iz2i2ziz","yiz2i2zix","xiz2i2ziy","ziz2i2zi"],
              "B":["ixr1zrx","xixr1zr"],
              "B'":["ix2xiy","xix2xix","yix2xi"],
              "B2":["ix2i2xix","xix2i2xi","ixr1r1zry","xixr1r1zrx","yixr1r1zr"]},

             {"R": ["zr1yrz"],"R'": ["z2x"],"R2":["z2i2yiz"],
              "L":["xr1yrx"],"L'":["x2z"],"L2":["x2i2yix"],
              "U":["yr1yr"],"U'":["y2y"],"U2":["y2i2yi"],
              "D":["r1yry"],"D'":["2"],"D2":["2i2yiy"],
              "F":["rx1xry","xrx1xrx","yrx1xr"],
              "F'":["rxi2zix","xrxi2zi"],
              "F2":["yrxi2i2zi","xrxi2i2zix","rxi2i2ziy","rx1r1xrx","xrx1r1xr"],
              "B":["rz1zry","xrz1zrx","yrz1zr"],
              "B'":["rzi2xiz","xrzi2xiy","yrzi2xix","zrzi2xi"],
              "B2":["rzi2i2xiy","xrzi2i2xix","yrzi2i2xi","rz1r1zrz","yrz1r1zrx","xrz1r1zry","zrz1r1zr"]}]

    nextState =[{"R": [1],"R'": [1],"R2":[0],
              "L":[1],"L'":[1],"L2":[0],
              "U":[1],"U'":[1],"U2":[0],
              "D":[1],"D'":[1],"D2":[0],
              "F":[0,0,0,0],
    ...
    for i in sol:
        b1 = b1 * len(moves[0][i])
        b2 = b2 * len(moves[1][i])
    print("number of strings is around",(b1+b2)/2)
    ...
    strReduced = reduce(i = 0,str = "iyr",state = 0)
    strReduced = strReduced[1:]
    print("number of steps:",nMoves(strReduced)+rots(strReduced)+hits(strReduced))
    print("number of hits:",hits(strReduced))
    print("number of rotations:",rots(strReduced))
    print("number of moves:",nMoves(strReduced))
    print("".join(strReduced))
    ser.write("".join(strReduced).encode('ascii'))
...
class RubiksVideo(RubiksOpenCV):
    def __init__(self, webcam):
        ...
        self.i = 0
        self.moves = ['x','x','x','rx','ixxr']
        self.delays = [4,4,4,7,10]
        # 0 for laptop camera, 1 for usb camera, etc
        self.webcam = webcam
    def delay(self,x):
        print("delay invoked")
        timer = Timer(x,self.true)
        timer.start()
    ...
    def analyze_webcam(self, width=352, height=240):
        time.sleep(2)
        rotateI = "pb"
        ser.write(rotateI.encode('ascii'))
        self.delay(14)
        time.sleep(8)
        ...
        while True:
            ...
            if (
                ...
                if(self.i<5):
                    ser.write(self.moves[self.i].encode('ascii'))
                    self.delay(self.delays[self.i])
                    self.i = self.i+1
                self.done = False
                ...
                if self.name == "F":
                    ...
                    # Run the solver
                        ...
                        cmd = (
                            "cd ~/rubiks-cube-NxNxN-solver/; ./rubiks-cube-solver.py --state %s"
                            %(kociemba_string)
                        )                                             
            ...
            if self.solution:
                ...
                if (
                    ...
                    slist = (self.solution).decode("utf-8").split()
                    row = 1
